...
static VALUE audio_play_se(int argc, VALUE* argv, VALUE self)
{
  ...
  name = rb_str_plus(rb_str("Audio/SE/"), name);
...
void audio_setup_custom_se()
{
  ...
  VALUE filename = rb_str("Data/System.rxdata");
  VALUE fileint = rb_const_get(rb_cIO, rb_intern("FileInt"));
  if (rb_funcall(fileint, rb_intern("exist?"), 1, filename) == Qfalse) return;
  VALUE ds = rb_funcall(rb_mKernel, rb_intern("load_data"), 1, filename);
  if ( RB_NIL_P(ds) ) return;
  rb_p(rb_str("Initializing extra Audio methods now..."));
...
void audioBindingInit()
{
  ...
  rb_iv_set(md, "@old_bgm_name", rb_str(""));
  rb_iv_set(md, "@old_bgs_name", rb_str(""));
  rb_iv_set(md, "@se", rb_str("Audio/SE/"));
