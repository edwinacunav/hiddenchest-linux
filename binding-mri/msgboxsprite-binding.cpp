...
static VALUE MsgBoxSpriteSetBitmap(VALUE self, VALUE bit)
{
  ...
  try {
    s->setBitmap(b);
  } catch (Exception &e) { Dbg() << "Could not create bitmap."; }
...
static VALUE MsgBoxSpriteSetOpacity(VALUE self, VALUE opacity)
{
  MsgBoxSprite *s = static_cast<MsgBoxSprite*>(RTYPEDDATA_DATA(self));
  if (!s) return Qnil;
  s->setOpacity( RB_FIX2INT(opacity) );
...
static VALUE MsgBoxSpriteSetBlendType(VALUE self, VALUE type)
{
  MsgBoxSprite *s = static_cast<MsgBoxSprite*>(RTYPEDDATA_DATA(self));
  if (!s) return Qnil;
  s->setBlendType( RB_FIX2INT(type) );
  return rb_iv_set(self, "blend_type", RB_INT2FIX( s->getBlendType() ));
}
...
static VALUE msgboxsprite_allocate(VALUE klass)
{
...
void MsgBoxSpriteBindingInit() {
  ...
  rb_define_alloc_func(RSBox, msgboxsprite_allocate<&MsgBoxSpriteType>);
