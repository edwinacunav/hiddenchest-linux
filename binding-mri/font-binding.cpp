...
static VALUE font_get_mirror(VALUE self)
{
  Font *f = static_cast<Font*>(RTYPEDDATA_DATA(self));
  if (!f) return Qnil;
...
static VALUE font_get_mirror_y(VALUE self)
{
  Font *f = static_cast<Font*>(RTYPEDDATA_DATA(self));
  if (!f) return Qnil;
...
VALUE linux_system_fonts_search()
{
  ...
  VALUE strings[] = {
    rb_str_plus(home, rb_str("/.local/share/fonts/*.{ttf,otf}")),
    rb_str("/usr/share/fonts/*.{ttf,otf}"),
    rb_str("/usr/share/fonts/*/*.{ttf,otf}"),
    rb_str("/usr/share/fonts/truetype/*/*.{ttf,otf}"),
    rb_str("/usr/share/fonts/opentype/*/*.{ttf,otf}"),
    rb_str("/usr/local/share/fonts/*/*.{ttf,otf}")
  };
  for (int n = 0; n < 6; n++)
...
void fontBindingInit()
{
  ...
  VALUE fonts;
  if (!strcmp(os_kind, "linux")) fonts = linux_system_fonts_search();
  ...
  VALUE default_names;
  if (defNames.empty()) {
    default_names = rb_str(DEFAULT_FONT);
  } else if (defNames.size() == 1) {
    if (defNames[0] == "Arial")
      default_names = rb_str(DEFAULT_FONT);
    else
      default_names = rb_str(defNames[0].c_str());
  } else {
    default_names = rb_ary_new2(defNames.size());
    for (size_t i = 0; i < defNames.size(); ++i)
      rb_ary_push(default_names, rb_str(defNames[i].c_str()));
  }
  ...
  rb_define_method(klass, "mirror", RMF(font_get_mirror), 0);
  rb_define_method(klass, "mirror=", RMF(font_set_mirror), 1);
  rb_define_method(klass, "mirror_y", RMF(font_get_mirror_y), 0);
  rb_define_method(klass, "mirror_y=", RMF(font_set_mirror_y), 1);
  rb_define_method(klass, "flip", RMF(font_get_mirror), 0);
  rb_define_method(klass, "flip=", RMF(font_set_mirror), 1);
  rb_define_method(klass, "flip_y", RMF(font_get_mirror_y), 0);
