...
static void write_log(const std::string &name, const std::string &log)
{
  std::ofstream file;
  file.open("error opengl.log");
  file << name + " Error!\n";
  file << log;
  file.close();
}
...
{
  INIT_SHADER(simple, gradient, GradientShader);
  ShaderBase::init();
  GET_U(origin);
  GET_U(resolution);
  GET_U(c1);
...
void GradientShader::set_origin(const Vec2i &ori)
{
  gl.Uniform2f(u_origin, ori.x / 100.0f, ori.y / 100.0f);
}

void GradientShader::set_resolution(float w, float h)
{
  gl.Uniform2f(u_resolution, w, h);
}

void GradientShader::set_colors(const Vec4 &c1, const Vec4 &c2)
{
  setVec4Uniform(u_c1, c1);
  setVec4Uniform(u_c2, c2);
}

GradientMapShader::GradientMapShader()
{
  INIT_SHADER(simple, gradient_map, GradientMapShader);
  ShaderBase::init();
  GET_U(opacity);
  GET_U(origin);
  GET_U(resolution);
...
{
  INIT_SHADER(simple, vignette, VignetteShader);
  ShaderBase::init();
  GET_U(resolution);
  GET_U(color);
}

void VignetteShader::set_resolution(float w, float h)
{
  gl.Uniform2f(u_resolution, w, h);
}

void VignetteShader::set_color(const Vec4 &color)
{
  setVec4Uniform(u_color, color);
}

SepiaVignetteShader::SepiaVignetteShader()
{
  INIT_SHADER(simple, vignette_sepia, SepiaVignetteShader);
  ShaderBase::init();
  GET_U(resolution);
  GET_U(color);
}
