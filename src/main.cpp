...
{
  Dbg() << "GL Vendor    :" << glGetStringInt(GL_VENDOR);
  Dbg() << "GL Renderer  :" << glGetStringInt(GL_RENDERER);
  Dbg() << "GL Version   :" << glGetStringInt(GL_VERSION);
  Dbg() << "GLSL Version :" << glGetStringInt(GL_SHADING_LANGUAGE_VERSION);
}
...
int rgssThreadFun(void *userdata)
{
  ...
  try {
    ...
    puts(dataDir);
    if (!strcmp(OS_STRING, "linux")) {
      int result = chdir("../");
      puts(SDL_GetBasePath());
...
{
  Dbg() << "HiddenChest The RGSS 1 2 & 3 Player Engine";
  Dbg() << "Developer:                  " << HIDDENAUTHOR;
  Dbg() << "HiddenChest Engine Version: " << HIDDENVERSION;
  Dbg() << "Release Date:               " << HIDDENDATE;
  const char *const makers[] = { "HiddenChest", "XP", "VX", "VX Ace" };
  //char buf[128];snprintf(buf, sizeof(buf),  %d (%s)", ver, makers[ver]);
  Dbg() << "Using RGSS Version" << ver << "(" << makers[ver] << ")";
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < 1000; ++i) {
    ...
    if (rtData.rqTermAck) {
      Dbg() << "RGSS thread acknowledged request after" << i*10 << "ms";
      break;
