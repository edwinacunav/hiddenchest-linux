...
static void fsys_initReadOps(PHYSFS_File *handle, SDL_RWops &ops, bool freeOnClose)
{
...
const uint32_t SDL_RWOPS_PHYSFS = SDL_RWOPS_UNKNOWN+10;
...
{
  delete p;
  if (PHYSFS_deinit() == 0)  Dbg() << "PhyFS failed to deinit.";
}
...
fontSetEnumCB (void *data, const char *dir, const char *fname)
{
  ...
  SDL_RWops ops;
  fsys_initReadOps(handle, ops, false);
...
openReadEnumCB(void *d, const char *dirpath, const char *filename)
{
  ...
  fsys_initReadOps(phys, data.ops, false);
...
void FileSystem::openReadRaw(SDL_RWops &ops, const char *fn, bool freeOnClose)
{
  ...
  if (!handle) Dbg() << fn << "file could not be found!";
  //assert(handle);// Fails if file doesn't exist, ignoring Ruby's rescue!
  fsys_initReadOps(handle, ops, freeOnClose);
}
