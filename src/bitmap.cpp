...
Bitmap::Bitmap(const char *filename, int none)
{
  ...
  if (!src) Dbg() << "No source!";
  SDL_Surface *surface = IMG_Load_RW(src, SDL_TRUE);
  if (!surface) Dbg() << "No surface";
  p = new BitmapPrivate(this);
...
void Bitmap::gradient_blend(int x, int y, SDL_Surface *s)
{
  guardDisposed();
  if (!s) return;//opacity = clamp(opacity, 0, 255);
  SDL_Surface *surf = p->surface;
  if (!surf) makeSurface();
}
...
void Bitmap::gradientFillRect(int x, int y, int width, int height,
                              const Vec4 &color1, const Vec4 &color2, int mode)
{
  gradientFillRect(IntRect(x, y, width, height), color1, color2, mode);
}
...
void Bitmap::invert_colors()
{
  ...
  int w = p->gl.width, h = p->gl.height, bpp = fmt->BytesPerPixel;
  uint8_t invert[w * bpp];
  for (int y = 0; y < h; y++) {
    for (int x = 0; x < w; x++) {
      ...
      invert[cx] = 255.0 - c.red;
      invert[cx + 1] = 255.0 - c.green;
      invert[cx + 2] = 255.0 - c.blue;
      invert[cx + 3] = c.alpha;
    }
    TEX::bind(p->gl.tex);
...
SDL_Surface* Bitmap::render_str(bool solid, const char *str, SDL_Color c)
{
  SDL_Surface *s;
  if (solid)
    s = TTF_RenderUTF8_Solid(p->font->getSdlFont(), str, c);
  else
    s = TTF_RenderUTF8_Blended(p->font->getSdlFont(), str, c);
  p->ensureFormat(s, SDL_PIXELFORMAT_ABGR8888);
  return s;
}
...
void Bitmap::drawText(const IntRect &rect, const char *str, int align)
{
  ...
  int opx = f->get_outline_size();
  int shapx = f->get_shadow_size();
  bool need_outline = f->get_outline();
  bool is_shadow = f->get_shadow();
  if (is_shadow) {
    if (need_outline) {
      SDL_Rect tmp = { opx, opx, surf->w, surf->h };
      SDL_Color out_color = f->get_out_color().toSDLColor();
      TTF_SetFontOutline(font, opx);
      SDL_Surface *other = render_str(is_solid, str, out_color);
      SDL_SetSurfaceBlendMode(surf, SDL_BLENDMODE_BLEND);
      SDL_BlitSurface(surf, 0, other, &tmp);
      SDL_FreeSurface(surf);
      surf = other;
    }
    SDL_Rect tmp;
    ...
    for (int n = 0; n < shapx; n++) {
      if (need_outline) TTF_SetFontOutline(font, opx);
      ...
      if (need_outline) TTF_SetFontOutline(font, 0);
      if (mode == 0) tmp.y += 1;
  ...
  if (fastBlit && !f->get_hflip() && !f->get_vflip()) {
    if (squeeze == 1.0f && !shState->config().subImageFix) {
      ...
      // the clipped visible part of it.  Dbg() << "Squeeze is equal to 1";
      SDL_Rect btmRect = { 0, 0, width(), height() };
      ...
      if (SDL_IntersectRect(&btmRect, &txtRect, &inters)) {
        ...
        if (inters.w != txtRect.w || inters.h != txtRect.h) {
          // Clip the text surface Dbg() << "Clipped!";
        ...
        TEX::bind(p->gl.tex);
        /*if (f->get_hflip())  h_flip_pixels(surf, irect);
        else if (f->get_vflip()) {
          SDL_Surface *s = flip_vertical(surf);
          SDL_FreeSurface(surf);
          surf = s;
        } else {*/
          if (!subImage) {// Dbg() << "No subimage";
    ...
    } else {// Squeezing involved: need to use intermediary TexFBO Dbg() << "Squeezed!";
      SDL_Rect btmRect = { 0, 0, width(), height() };
      SDL_Rect txtRect = { irect.x, irect.y, irect.w, irect.h };
      SDL_Rect inters;
      if (SDL_IntersectRect(&btmRect, &txtRect, &inters)) {
    ...
    TEX::setSmooth(true);
    Quad &quad = shState->gpQuad();
    FloatRect rt = posRect;
    if (f->get_hflip()) {
      rt.x -= alignX;
      rt.y = 0;
      quad.setTexRect(rt.hFlipped());
    } else if (f->get_vflip()) {
      rt = { posRect.x - 4, rect.y + 4.0f, posRect.w, posRect.h };
      quad.setTexRect(rt.wFlipped());
    } else {
