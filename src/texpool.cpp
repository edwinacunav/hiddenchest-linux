...
// Dbg() << "TexPool: <!#> (" << obj.width << obj.height << ")";
    TEXFBO::fini(obj);
    return;
  }
...
// Dbg() << "TexPool: <!~> Size:" << p->memSize;
    ...
    --p->objCount;
// Dbg() << "TexPool: <!-> (" << last.obj.width << last.obj.height << ")";
  }
  ...
  ++p->objCount;
// Dbg() << "TexPool: <!+> (" << obj.width << obj.height << ") Current size:" << p->memSize;
}
