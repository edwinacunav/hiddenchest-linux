...
void SharedFontState::init_system_font(const std::string &filename)
{
  ...
  //if (p->sys_fonts.contains(family)) Dbg() << "SysF" << p->sys_fonts[family];
  FontKey key(family, 22);
  p->pool.insert(key, font);
  TTF_Font *tmp = p->pool.value(key);//if (tmp) Dbg() << "Font is still opened!";
  FontSet &set = p->sets[family];
  if (style == "Regular")
    set.regular = filename;
  else
    set.other = filename;//Dbg() << set.regular << set.other;
  sys_fonts_size++;
  return;
}

int SharedFontState::get_local_fonts_size()
{
  return local_fonts_size;
}

int SharedFontState::get_sys_fonts_size()
{
  return sys_fonts_size;
}
...
_TTF_Font *SharedFontState::getFont(std::string family, int size)
{
  ...
  if (family.size() > 0) {
    //Dbg() << "Quick search" << family << "- Size:" << size;
  ...
  // Find out if the font asset exists Dbg() << "Get" << family;//reg;
  ...
  if (!loadf.empty() ) {
    ...
    //if (family.size() > 0) {
    font = p->pool.value(key);
    if (font) return font;//Dbg() << "Second search failed!";
    //}
    ...
    if (f) {
      ...
      p->pool.insert(key, font);
      if (font) return font;//Dbg() << "Font loaded!?";
    } else {//Dbg() << "No raw file, no pool, no font!?";
      SDL_RWclose(f);
    }
  ...
  if (family.empty()) {// Built-in font
    Dbg() << "No font found or requested, loading FreeSans by default";
    ops = openBundledFont();
  } else {
    std::string reg = req.regular;// Dbg() << "Font family found:" << reg;
...
std::string FontPrivate::default_name = OS_STRING == "linux" ? "FreeSans" : "Arial";
...
void Font::initDefaults(const SharedFontState &sfs)
{
  ...
  case 1 :
    ...
    names.push_back(OS_STRING == "linux" ? "FreeSans" : "Arial");
    break;
  case 2 :
...
void Font::init_system_fonts(const std::vector<std::string> &names)
{
  ...
  Dbg() << "Found" << shState->fontState().get_sys_fonts_size() << "system fonts on your OS";
}
